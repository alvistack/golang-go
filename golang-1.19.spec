...
Provides: golang-1.19-devel = %{epoch}:%{version}-%{release}
Provides: golang-1.19-devel-static = %{epoch}:%{version}-%{release}
...
set -ex && \
    ...
    export GOROOT_FINAL=%{_libdir}/go-1.19 && \
    cd ./src && \
    bash ./make.bash

%install
install -Dpm755 -d %{buildroot}%{_bindir}
install -Dpm755 -d %{buildroot}%{_libdir}/go-1.19
install -Dpm755 -d %{buildroot}%{_docdir}/go-1.19/html
cp -rfp VERSION %{buildroot}%{_libdir}/go-1.19/
cp -rfp api %{buildroot}%{_libdir}/go-1.19/
cp -rfp bin %{buildroot}%{_libdir}/go-1.19/
cp -rfp doc/* %{buildroot}%{_docdir}/go-1.19/html/
cp -rfp misc %{buildroot}%{_libdir}/go-1.19/
cp -rfp pkg %{buildroot}%{_libdir}/go-1.19/
cp -rfp src %{buildroot}%{_libdir}/go-1.19/
cp -rfp test %{buildroot}%{_libdir}/go-1.19/
find %{buildroot}%{_libdir}/go-1.19/src -type f -name '*.rc' -delete
ln -fs %{_libdir}/go-1.19/bin/go %{buildroot}%{_bindir}/go
ln -fs %{_libdir}/go-1.19/bin/gofmt %{buildroot}%{_bindir}/gofmt
