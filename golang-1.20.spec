...
set -ex && \
    ...
    export GOROOT_FINAL=%{_libdir}/go-1.20 && \
    cd ./src && \
    bash ./make.bash

%install
install -Dpm755 -d %{buildroot}%{_bindir}
install -Dpm755 -d %{buildroot}%{_libdir}/go-1.20
install -Dpm755 -d %{buildroot}%{_docdir}/go-1.20/html
cp -rfp VERSION %{buildroot}%{_libdir}/go-1.20/
cp -rfp api %{buildroot}%{_libdir}/go-1.20/
cp -rfp bin %{buildroot}%{_libdir}/go-1.20/
cp -rfp doc/* %{buildroot}%{_docdir}/go-1.20/html/
cp -rfp misc %{buildroot}%{_libdir}/go-1.20/
cp -rfp pkg %{buildroot}%{_libdir}/go-1.20/
cp -rfp src %{buildroot}%{_libdir}/go-1.20/
cp -rfp test %{buildroot}%{_libdir}/go-1.20/
find %{buildroot}%{_libdir}/go-1.20/src -type f -name '*.rc' -delete
ln -fs %{_libdir}/go-1.20/bin/go %{buildroot}%{_bindir}/go
ln -fs %{_libdir}/go-1.20/bin/gofmt %{buildroot}%{_bindir}/gofmt
...
BuildArch: noarch
Summary: Go programming language
Requires: golang-1.20 = %{epoch}:%{version}-%{release}
